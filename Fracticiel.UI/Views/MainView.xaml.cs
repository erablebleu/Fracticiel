...
public partial class MainView : Window
{
    ...
    private const int _multisampling = 8;
    ...
    private double _res = 0.074942 / _w;

    private double _x0 = -1.41294724;

    private double _y0 = -0.037471;
    ...
    private void Draw()
    {
        ...
        {
            Width = _w * _multisampling,
            Height = _h * _multisampling,
            X = _x0,
            Y = _y0,
            Resolution = _res / _multisampling,
        };
        ...
        sw01.Start();
        //arr = Fracticiel.Calculator.Mandelbrot.GPUInvoke(dataBlock, mSet);
        arr = Fracticiel.Calculator.Buddhabrot.GPUInvoke(dataBlock, bSet);
        //arr = Fracticiel.Calculator.Julia.GPUInvoke(dataBlock, jSet);

        sw01.Stop();

        if (_multisampling > 1)
            arr = Fracticiel.Calculator.MultiSampling.GPUInvoke(arr, _w, _h, _multisampling);
        //Cuda.Calculator.Multisampling(ref samples, arr, _w, _h, _multisampling);

        if (false)
        {
            colorsRGB = Calculator.Colorizer.RGB(arr, 0, 230);
            image.Source = BitmapFromDataRGB((int)_w, (int)_h, colorsRGB);
        }
        else
        {
            sw02.Start();
            colorsBW = Fracticiel.Calculator.Colorizer.BW(arr, 0, 255);
