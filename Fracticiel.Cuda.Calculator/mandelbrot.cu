...
__global__ void kernel_mandelbrot(uint32_t* result, int32_t dW, int32_t dH, double x0, double y0, double res, int32_t maxLoopCnt, double maxMagnitude) {
...
int32_t mandelbrot(uint32_t* result, const DataBlock* block, const Settings_Mandelbrot* settings) {
   ...
   kernel_mandelbrot << <numBlocks, threadsPerBlock >> > (cuda_result, block->Width, block->Height, block->X, block->Y, block->Resolution, settings->LoopCount, settings->Magnitude);
